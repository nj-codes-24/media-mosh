...
function Camera({ onSelect }: { onSelect: (t: MediaType) => void }) {
  ...
  return (
    <div className="camera-wrapper" style={{ position: 'relative', width: DW, height: DH }}>
...
function Landing({ onEnter }: { onEnter: (t: MediaType) => void }) {
  return (
    ...
    >
      ...
      <motion.nav className="landing-nav" initial={{ opacity: 0, y: -14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.5 }}
      ...
      <div className="landing-content" style={{ flex: 1, display: 'flex', alignItems: 'center', position: 'relative', zIndex: 20, minHeight: 0, padding: '0 44px', overflow: 'hidden' }}>
        <div className="landing-text" style={{ width: '38%', flexShrink: 0 }}>
          <motion.div className="subtitle-line" initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.18, duration: 0.5 }}
          ...
          {/* ── MOBILE CTA GRID (visible only on ≤768px, replaces camera) ──── */}
          <motion.div className="mobile-cta-grid"
          ...
          >
            {CATS.map((cat, i) => (
              <motion.button
                key={cat.type}
                whileTap={{ scale: 0.95 }}
                onClick={() => onEnter(cat.type)}
                className="mobile-cta-btn"
              >
                <cat.icon style={{ width: 20, height: 20, color: '#FF4400', flexShrink: 0 }} />
                <div>
                  <div style={{ fontSize: 14, fontFamily: 'Bebas Neue,sans-serif', letterSpacing: '0.12em', color: 'rgba(255,255,255,0.9)', lineHeight: 1 }}>
                    {cat.label}
                  </div>
                  <div style={{ fontSize: 7, fontFamily: 'DM Mono,monospace', letterSpacing: '0.15em', color: 'rgba(255,255,255,0.25)', textTransform: 'uppercase', marginTop: 4 }}>
                    {cat.desc}
                  </div>
                </div>
              </motion.button>
        ...
        <div className="camera-section" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', minWidth: 0, position: 'relative' }}>
      ...
      <div className="landing-marquee" style={{ position: 'relative', zIndex: 20, borderTop: '1px solid rgba(255,255,255,0.05)', padding: '11px 0', flexShrink: 0 }}>
        <Marquee />
      </div>
    </motion.div>
  );
...
function Dashboard({ activeTab, setActiveTab, selectedTool, setSelectedTool, processor, setProcessor, searchQuery, setSearchQuery, isLoading, setIsLoading, onHome }: {
  ...
  return (
    ...
    >
      ...
      {/* ══ SIDEBAR ══════════════════════════════════════════════════════════ */}
      <aside className={`dash-sidebar ${mobileSidebarOpen ? 'mobile-open' : ''}`} style={{
        width: 228,
        ...
        <div className="cscroll" style={{ flex: 1, overflowY: 'auto', padding: '2px 0 24px', display: 'flex', flexDirection: 'column', gap: 0 }}>
          {toolRegistry.filter(t => t.category === activeTab).map(tool => {
            ...
            return (
              <button key={tool.id} disabled={soon} onClick={() => { if (!soon) { handleToolSelect(tool); setMobileSidebarOpen(false); } }}
                style={{
      ...
      {/* ══ MAIN CONTENT ═════════════════════════════════════════════════════ */}
      <main style={{
        flex: 1, overflowY: 'auto', position: 'relative', display: 'flex', flexDirection: 'column',
        ...
        <div className="dash-topbar" style={{
          position: 'relative', zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 32px', borderBottom: '1px solid rgba(255,255,255,0.07)', flexShrink: 0,
          ...
          <AnimatePresence mode="wait">
            {!selectedTool ? (
              <motion.div key={`grid-${activeTab}`}
                ...
                className="dash-grid-wrap"
                style={{ padding: '26px 32px 48px' }}
              >

                {/* Fresh, New Creative Category Text Style */}
                <div className="dash-category-header" style={{ marginBottom: 36, display: 'flex', alignItems: 'center' }}>
                  <motion.div
                    initial={{ height: 0 }}
                    animate={{ height: 50 }}
                ...
                <div className="dash-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
